/*
  MPU-6050 & MS5611 Controlled 3-DOF Stewart Platform
  ...
  - MPU SCL: Pin A5
  - MPU SDA: Pin A4
  - MS5611 SCL: Pin A5 (Shared)
  - MS5611 SDA: Pin A4 (Shared)
*/
...
void setup() {
  Serial.begin(115200);
  Serial.println("Stewart Platform MPU + Barometer Controller Initializing...");
  ...
  // 3. CALIBRATE BAROMETER
  // We must find the "0" altitude to use as our reference.
...
void rotateRPY(const Vec3 &in, Vec3 &out, float roll, float pitch, float yaw) {
  ...
  float cy = cos(yaw), sy = sin(yaw); // Note: 'copy' was a typo here, fixed to 'yaw'
